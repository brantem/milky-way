  ...
  return (
    <>
      <div className="p-3 h-full w-full flex">
        <div className="relative flex-1 flex flex-col items-center justify-center bg-white rounded-lg shadow-sm border border-neutral-200/50">
          <h1 className="text-5xl font-bold mb-6">{solarSystem.title}</h1>

          <Link to={firstPlanet ? `/${solarSystem.id}/${firstPlanet.id}` : '/'}>
            <Button shadowClassName="bg-sky-600" contentClassName="bg-sky-500 text-white px-4" disabled={!firstPlanet}>
              Start{' '}
            </Button>
          </Link>
          ...
          <div className="absolute right-3 bottom-3 left-3 flex items-end justify-between gap-3">
            ...
            <div className={cn('flex items-center gap-1 transition-opacity', !offline.ready && 'opacity-0')}>
              <span className="text-neutral-500">Ready for offline use</span>
              <svg
