...
const Jupiter = () => {
  ...
  return (
    <Planet<Jupiter> path="planets/jupiter/_planet.json">
      {({ planet, onRequest, onChange }) => {
        const parent: Parent = {
          id: planet.id,
          request: onRequest,
        };
                      ...
                      <div className="flex justify-between gap-2 p-2">
                        ...
                        <div className="flex gap-2">
                          {actions.submit && (
                            <SubmitButton
                              onClick={() => {
                                if (smallRef.current?.snapshot)
                                  handleSnapshot(planet.small.id, smallRef.current.snapshot());
                                if (mediumRef.current?.snapshot)
                                  handleSnapshot(planet.medium.id, mediumRef.current.snapshot());
                                if (largeRef.current?.snapshot)
                                  handleSnapshot(planet.large.id, largeRef.current.snapshot());
                              }}
                            />
                          )}
                        </div>
